/*********************************************************/
/**                                                     **/
/**   UVa 00190 - Circle Through Three Points.cpp       **/
...
void printCircleEquation1(point C, double r)
{
    ...
    printf("(x ");
    if(h>=0)        printf("- %0.3lf",h);
    else             printf("+ %0.3lf",-h);
    printf(")^2 + (y ",h);
    if(k>=0)        printf("- %0.3lf",k);
    else             printf("+ %0.3lf",-k);
    printf(")^2 = %0.3lf^2\n",r);

}

// x^2 + y^2 + cx + dy - e = 0      // c = -2*h , d = -2*k , e = -(h^2+k^2-r^2)
void printCircleEquation2(point C, double r)
{
    double h = C.X;
    double k = C.Y;
    ...
    printf("x^2 + y^2 ");
    if(c>=0)     printf("+ %0.3lfx ",c);
    else         printf("- %0.3lfx ",-c);
    if(d>=0)     printf("+ %0.3lfy ",d);
    else         printf("- %0.3lfy ",-d);
    if(e>=0)     printf("- %0.3lf",e);
    else         printf("+ %0.3lf",-e);
    printf(" = 0\n");
}
...
int main()
{
    ...
    while(cin >> x1 >> y1 >> x2 >> y2 >> x3 >> y3)
    {
        ...
        printCircleEquation1(circle.second,circle.first);
        printCircleEquation2(circle.second,circle.first);
        printf("\n");
    }
    return 0;
}
